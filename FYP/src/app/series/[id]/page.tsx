...
export default function SeriesDetailPage() {
  ...
  const heroStyle = useMemo(() => {
    const hex = series?.ambientColorHex ?? "#7C3AED";
    return {
      background:
        `radial-gradient(900px 520px at 20% 10%, ${hex}55, transparent 60%),` +
        `radial-gradient(900px 520px at 80% 20%, rgba(245,158,11,0.14), transparent 60%),` +
        "linear-gradient(180deg, rgba(0,0,0,0.15), rgba(0,0,0,0.72))"
    } as const;
  }, [series?.ambientColorHex]);
  ...
  return (
    <div className="min-h-dvh">
      <div className="relative overflow-hidden border-b border-white/8 bg-surface">
        ...
        <div className="relative mx-auto grid max-w-5xl grid-cols-1 gap-6 px-4 py-8 md:grid-cols-[220px_1fr] md:py-10">
          <div
            className="aspect-[3/4] w-[220px] overflow-hidden rounded-2xl border border-white/10 bg-cover bg-center shadow-2xl"
            style={{ backgroundImage: `url(${series.coverUrl})` }}
          />

          <div className="space-y-3">
            <h1 className="font-display text-4xl tracking-widest md:text-5xl">{series.title}</h1>
          ...
          <div className="space-y-3">
            {chapters.map((c) => {
              ...
              return (
                <div
                  key={c.id}
                  className="flex flex-col gap-3 rounded-2xl border border-white/10 bg-card p-4 md:flex-row md:items-center md:justify-between"
                >
                  <div>
